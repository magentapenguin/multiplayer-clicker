...
export default class Server implements Party.Server {
    ...
    getScaledPrice(item: ShopItem) {
        return (
            item.price *
            Math.pow(item.priceScale, this.purchasedItems[item.id] ?? 0)
        );
    }
    ...
    onConnect(
        ...
        this.users++;
        this.room.broadcast(
            JSON.stringify({ type: "users", users: this.users })
        );
    ...
    applyStats() {
        ...
        this.room.broadcast(
            JSON.stringify({ type: "stats", stats: this.stats })
        );
    }
    rateLimit(connection: Party.Connection<{ lastMsg: number }>) {
        const now = performance.now();
        if (
            connection.state?.lastMsg &&
            connection.state?.lastMsg > now - (rateLimit - this.stats.rateLimit)
        ) {
            connection.send(
                JSON.stringify({
                    type: "error",
    ...
    async onMessage(
        ...
        if (data.type === "click") {
            ...
            const action = (item: ShopItem) => {
                if (item.action === "addPoints") {
                    ...
                    this.purchasedItems[item.id] =
                        (this.purchasedItems[item.id] ?? 0) + 1;
                    this.applyStats();
                }
            };
            ...
            if (!this.rateLimit(sender)) return;
            const message = profanityCensor.applyTo(
                data.message,
                profanityFilter.getAllMatches(data.message)
            );
            // prevent xss
            ...
            this.room.broadcast(
                JSON.stringify({ ...data, sender: sender.id }),
                [sender.id]
            );
        }
    }
