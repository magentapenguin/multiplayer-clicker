...
const shop = new Shop(
    document.getElementById("shop-items") as HTMLElement,
    (itemId: string) => {
...
party.addEventListener("message", (event) => {
    ...
    if (data.type === "clicks") {
        ...
        toast("Error", data.message, {
            duration: parseInt(data.retryIn) * 1000,
            type: "danger",
        ...
        toast(
            data.sender + (data.sender === party.id ? " (you)" : ""),
            data.message,
            {
                duration: -1,
                type: "none",
...
function updateConnection() {
    document.body.classList.toggle(
        "loading",
        party.readyState === PartySocket.CONNECTING
    );
    document.body.classList.toggle(
        "connected",
        party.readyState === PartySocket.OPEN
    );
}
